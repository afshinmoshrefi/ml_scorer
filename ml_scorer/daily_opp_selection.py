...
Selection pipeline:
  ...
  6. Deduplicate: keep best candidate per symbol (highest sharpe_ratio)
...
try:
    from .scorer_config import DATA_DIR, ML_PARQUET_MARKETS, TIERS
except ImportError:
    from scorer_config import DATA_DIR, ML_PARQUET_MARKETS, TIERS
...
def load_candidates(date, resource_ids, direction, days_out_min, days_out_max,
    ...
    # Dedup: keep the best candidate per symbol.
    # "Best" = highest sharpe_ratio among all combos and daysOut values.
    # This reduces hundreds of rows per symbol down to one.
    df = df.sort_values('sharpe_ratio', ascending=False)
    df = df.drop_duplicates(subset='sym', keep='first')

    log.info(f'Pre-filter: {len(df)} unique symbols after filtering')
    return df
...
def select_daily_opps(date, resource_ids, num_picks, direction, days_out_min,
    ...
    Args:
        ...
        resource_ids: list of market resource IDs to search (e.g. ['2', '11'])
