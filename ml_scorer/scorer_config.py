"""Configuration for ML Pattern Scorer service.

Paths are imported from the central /home/flask/config.py on Linux servers.
On Windows (dev), paths fall back to local defaults since the central config
doesn't exist there. This prevents deploying the ml_scorer folder from ever
overwriting the central config.
"""
import os
import sys

# Try to import paths from the central TradeWave config.
# On Linux servers (dev/staging/prod), this is the single source of truth.
# On Windows dev, this import fails and we fall back to local defaults.
_parent_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
if _parent_dir not in sys.path:
    sys.path.insert(0, _parent_dir)

try:
    from config import ddir as _ddir, csv_folder as _csv_folder
    DATA_DIR = _ddir.rstrip('/')
    CSV_DIR = _csv_folder.rstrip('/')
    EARNINGS_DIR = os.path.join(os.path.dirname(DATA_DIR), 'edgar', 'earnings')
except ImportError:
    # Windows dev -- no central config.py available
...
# Markets to generate daily ML parquet caches for.
# Maps resource_id -> (display_name, data_folder_name).
# The data_folder_name is the subdirectory under DATA_DIR that contains
# the opportunities/ directory (and optionally opp_by_symbol/).
# The parquet file is written to: DATA_DIR/<folder>/ml_cache_<date>.parquet
ML_PARQUET_MARKETS = {
    '0':  ('DOW 30',        'dj30'),
