"""
Feature Engine for ML Pattern Scorer.

Computes all 73 features for any (symbol, date, daysOut, direction) opportunity.
5 feature groups:
  1. Pattern-Intrinsic (23) - historical quality and depth profile
  2. Technical Momentum (20) - current market momentum / TA
  3. Market Regime (14) - broad market conditions
  4. Stock-Specific Context (9) - what makes this stock unique now
  5. Calendar/Cycle (7) - time-based patterns

Usage:
  engine = FeatureEngine()
  engine.load_price_data(['AAPL', 'MSFT'])  # load needed tickers
...
from config_ml import (
    ...
    get_pe_year, SPX_SEASONAL_CUTOFF_YEAR, SPX_SEASONAL_FORWARD_DAYS,
    CSV_DIR
)
...
class FeatureEngine:
    """Computes all 73 features for pattern opportunities."""
    ...
    def _find_csv_path(self, symbol):
        ...
        for subdir in ['US', 'ETF', 'INDX', 'COMM']:
            path = os.path.join('/home/flask/data/csv', subdir, f'{symbol}.csv')
    ...
    # ------------------------------------------------------------------
    # Group 1: Pattern-Intrinsic Features (23)
    # ------------------------------------------------------------------
    ...
    def compute_pattern_features(self, symbol, date, daysOut, direction):
        ...
        if best_row is None:
            ...
            return {f'pat_{k}': np.nan for k in [
    ...
    def compute_technical_features(self, symbol, date):
        ...
        if df is None:
            return {f'ta_{k}': np.nan for k in [
    ...
    # ------------------------------------------------------------------
    # Group 3: Market Regime Features (14)
    # ------------------------------------------------------------------

    def compute_market_regime_features(self, date):
        """
        Group 3: Market Regime features (14).
        Broad market conditions from VIX, bonds, credit, SPY, breadth.
        """
        features = {}
        if isinstance(date, str):
            date = pd.Timestamp(date)
        ...
        # Breadth approximation: % of S&P 500 stocks above SMA50
        # This is expensive to compute for training; use ADVN/DECN ratio as proxy
    ...
    def compute_features(self, symbol, date, daysOut, direction):
        """
        Compute all 73 features for a single opportunity.
    ...
    def get_feature_names(self):
        """Return ordered list of all 73 feature names."""
        return [
            # Group 1: Pattern-Intrinsic (23)
